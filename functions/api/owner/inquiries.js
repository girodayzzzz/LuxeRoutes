...
export const onRequestGet = async ({ request, env }) => {
  try {
    ...
    const statement = auth.role === 'admin'
      ...
      : auth.db.prepare(`${inquirySelect} WHERE lower(trim(owner_email)) = ? AND forwarded_to_owner_at IS NOT NULL ORDER BY created_at DESC LIMIT 100`).bind(auth.email);
...
export const onRequestPatch = async ({ request, env }) => {
  try {
    ...
    if (auth.role !== 'admin' && String(inquiry.ownerEmail || '').trim().toLowerCase() !== auth.email) {
      return privateErrorJson('This inquiry is not assigned to your owner account.', 403);
    }
    ...
    return privateErrorJson(error.message || 'Unable to update owner inquiry.', 500);
  }
};

...
## Build troubleshooting — deployment fails during initialization

If Cloudflare Pages fails immediately at **Initializing build environment**, first check `wrangler.toml`. Do **not** commit a placeholder D1 binding such as `database_id = "REPLACE_WITH_CLOUDFLARE_D1_DATABASE_ID"`. Cloudflare validates Wrangler configuration before the site deploys, so an invalid D1 ID can stop the build before your HTML files are published.

The safe order is:

1. Deploy the Pages project with only the basic `wrangler.toml` settings.
2. Create the D1 database.
3. Add the D1 binding in the Cloudflare Pages dashboard with variable name `DB`.
4. Redeploy.
...
After the command succeeds, Cloudflare returns a database UUID. For this Pages setup, the recommended production path is to bind the database in the Cloudflare dashboard instead of committing a placeholder D1 block to `wrangler.toml`.

If you still want a local Wrangler binding, add this block only after replacing `YOUR_REAL_DATABASE_ID` with the real UUID returned by Cloudflare:
...
Never commit `REPLACE_WITH_CLOUDFLARE_D1_DATABASE_ID`; it can break Cloudflare Pages during build initialization.

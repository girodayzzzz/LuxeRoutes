...
Admins can then grant `customer`, `owner`, `manager`, or `admin` access by verified email after D1 is created, the `DB` binding is connected, and the first admin row is seeded. If Cloudflare Pages fails during build initialization, confirm that `wrangler.toml` does not contain a placeholder D1 `database_id`. See [`docs/cloudflare-admin-auth.md`](docs/cloudflare-admin-auth.md) for the full Cloudflare + D1 plan.

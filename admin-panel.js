...
const normalizeProfileRole = (profile) => ['customer', 'owner', 'manager'].includes(profile?.requestedRole)
...
const renderRegistrationCard = (profile) => {
  const requestedRole = normalizeProfileRole(profile);
  const activeRole = getProfileActiveRole(profile);
  const companySummary = profileCompanySummary(profile);
  const canReviewProfile = canApprove() && ['owner', 'manager'].includes(requestedRole) && profile.status === 'pending_admin_grant' && !profile.localOnly;

  return `
    <div class="registration-card">
      <div class="registration-card-head">
        <div>
          <strong>${escapeHtml(profile.name || profile.email || 'Unnamed profile')}</strong>
          <span>${escapeHtml(profile.email || 'Email pending')}</span>
        </div>
        <span class="status-pill ${profileStatusClass(profile.status)}">${escapeHtml(profileStatusLabel(profile.status))}</span>
      </div>
      <div class="registration-meta">
        <span>Requested: ${escapeHtml(formatRole(requestedRole))}</span>
        <span>Current: ${escapeHtml(formatRole(activeRole))}</span>
        ${companySummary ? `<span>${escapeHtml(companySummary)}</span>` : ''}
        ${profile.companyWebsite ? `<span>${escapeHtml(profile.companyWebsite)}</span>` : ''}
        ${profile.notes ? `<span>${escapeHtml(profile.notes)}</span>` : ''}
        ${profile.localOnly ? '<span>Local preview only — submit through production D1 before approval.</span>' : ''}
      </div>
      ${canReviewProfile ? `
        <div class="access-request-actions">
          <button class="mini-action" type="button" data-pending-role-action="approve" data-pending-email="${escapeHtml(profile.email)}" data-pending-role="${escapeHtml(requestedRole)}">Approve</button>
          <button class="mini-action mini-action-warning" type="button" data-pending-role-action="reject" data-pending-email="${escapeHtml(profile.email)}" data-pending-role="${escapeHtml(requestedRole)}">Reject</button>
        </div>
      ` : ''}
    </div>
  `;
...
const renderPeople = () => {
  ...
  if (registrationList) {
    ...
    const localProfiles = isLocalPreview() && accountProfile?.email
      ? [{ ...accountProfile, id: 'local-preview-profile', localOnly: true }]
      : [];
    const profiles = [...remoteProfiles, ...localProfiles]
      .filter((profile, index, list) => profile?.email && list.findIndex((item) => item.email === profile.email) === index);
    const roles = ['customer', 'owner', 'manager'];

    registrationList.innerHTML = profiles.length
      ? roles.map((role) => {
        const roleProfiles = profiles.filter((profile) => normalizeProfileRole(profile) === role);
        const pendingCount = roleProfiles.filter((profile) => profile.status === 'pending_admin_grant').length;
        return `
          <article class="registration-column">
            <div class="registration-column-head">
              <div>
                <p class="eyebrow">${escapeHtml(formatRole(role))}</p>
                <strong>${roleProfiles.length} registrations</strong>
              </div>
              <span class="status-pill ${pendingCount ? 'status-pending' : 'status-approved'}">${pendingCount} pending</span>
            </div>
            <div class="registration-card-list">
              ${roleProfiles.map(renderRegistrationCard).join('') || '<p class="empty-state">No registrations yet.</p>'}
            </div>
          </article>
        `;
      }).join('')
      : '<p class="empty-state">No customer, owner, or manager registrations in D1 yet.</p>';
  }
